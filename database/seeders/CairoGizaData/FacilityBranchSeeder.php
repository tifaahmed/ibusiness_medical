...
class FacilityBranchSeeder extends Seeder
{
    ...
    public function run(): void
    {
        try {
            ...
            $this->command->info("Starting branch import for Cairo/Giza...");
    ...
    {
        // Handle location - used for branch name/identifier
        $location = '';
        $locationAr = '';
        $locationEn = '';
        
        // Handle address - used for branch address field
        $addressAr = '';
        $addressEn = '';
        ...
        // Check for 'address' field - this is the actual address
        if (!empty($branchData['address'])) {
            if (is_array($branchData['address'])) {
                $addressAr = (string) ($branchData['address']['ar'] ?? '');
                $addressEn = (string) ($branchData['address']['en'] ?? $addressAr);
            } else {
                $addressAr = (string) $branchData['address'];
                $addressEn = $addressAr;
            }
        }
        
        // If no location but we have address, use address for location too
        if (empty($location) && !empty($addressAr)) {
            $location = $addressAr;
            $locationAr = $addressAr;
        ...
        $addressAr = $addressAr ?: $locationAr;
