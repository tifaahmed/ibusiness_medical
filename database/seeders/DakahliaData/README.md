# Dakahlia Data Seeders

This folder contains seeders for importing medical facilities data for Dakahlia governorate from JSON format.

## Structure

- `GovernorateSeeder.php` - Creates Dakahlia governorate
...
- `DakahliaDataSeeder.php` - Main seeder that runs all seeders in correct order
- `dakahlia.json` - JSON data file containing all facility information in nested associated array format
      ...
      "facilities": [
        {
          ...
          "branches": [
            // Can be array of strings: ["Mansoura", "El Manzala"]
            // Or array of objects:
            {
...
```bash
php artisan db:seed --class=Database\\Seeders\\DakahliaData\\DakahliaDataSeeder
```
...
```bash
# Create governorate
php artisan db:seed --class=Database\\Seeders\\DakahliaData\\GovernorateSeeder

# Create facility types
php artisan db:seed --class=Database\\Seeders\\DakahliaData\\FacilityTypeSeeder

# Create facilities
php artisan db:seed --class=Database\\Seeders\\DakahliaData\\FacilitySeeder

# Create branches
php artisan db:seed --class=Database\\Seeders\\DakahliaData\\FacilityBranchSeeder
```
...
- **Branches Array**: Supports both string arrays (like `["Mansoura", "El Manzala"]`) and object arrays
...
- **Specialty Field**: Facilities can have a `specialty` field (for doctors/clinics), though it's not currently stored in the database schema
