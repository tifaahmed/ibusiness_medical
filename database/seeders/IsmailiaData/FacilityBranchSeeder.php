...
class FacilityBranchSeeder extends Seeder
{
    ...
    public function run(): void
    {
        try {
            ...
            $this->command->info("Starting branch import for Ismailia...");
                ...
                foreach ($category['facilities'] as $facilityData) {
                    ...
                    if (!empty($branches) && is_array($branches)) {
                        foreach ($branches as $branchIndex => $branchData) {
                            ...
                            // Handle address - can be string, object with ar/en, or in 'address' field
                            $location = '';
                            $locationAr = '';
                            $locationEn = '';
                            
                            // Check for 'address' field first (Assiut/Ismailia format)
                            if (!empty($branchData['address'])) {
                                if (is_array($branchData['address'])) {
                                    // Address is an object with ar/en
                            ...
                            } elseif (!empty($branchData['location'])) {
                                // Check for 'location' field (Aswan format)
                            ...
                            // Ensure we always have valid strings (not null or empty)
                            ...
                            // Use locationAr for Arabic and locationEn for English
                            $branchNameAr = '';
                            $branchNameEn = '';
                            ...
                            $branchNameAr = (string) $branchNameAr;
    ...
    {
        $jsonFilePath = base_path('database/seeders/IsmailiaData/ismailia.json');
