...
class FacilityBranchSeeder extends Seeder
{
    ...
    public function run(): void
    {
        try {
            ...
            $this->command->info("Starting branch import for Alexandria...");
    ...
    protected function processFacilityBranches(array $facilityData, &$created, &$updated): void
    {
        ...
        if (!empty($branches) && is_array($branches)) {
            foreach ($branches as $branchIndex => $branchData) {
                ...
                if (is_string($branchData)) {
                    ...
                    // Convert phone to array format
                    $phoneArray = !empty($branchPhone) ? [trim($branchPhone)] : null;
        ...
        if (count($phones) > 1 && empty($branches)) {
            ...
            for ($i = 1; $i < count($phones); $i++) {
                ...
                // Convert phone to array format
                $phoneArray = !empty($phone) ? [trim($phone)] : null;
        ...
        if ($currentBranchesCount === 0) {
            // Create main branch using facility data from JSON (not from facility model)
    ...
    protected function createBranchFromObject($facility, array $facilityData, array $branchData, int $branchIndex, &$created, &$updated): void
    {
        ...
        if (count($branchPhones) > 1) {
            ...
            // Keep branchPhones as array format (already an array)
            $branchPhonesArray = !empty($branchPhones) ? array_map('trim', array_filter($branchPhones, fn($p) => !empty($p))) : null;
            $branchPhone = !empty($branchPhonesArray) ? $branchPhonesArray[0] : null;
            ...
            if (empty($branchPhonesArray)) {
                if (!empty($facilityData['hotline'])) {
                    $branchPhonesArray = [trim($facilityData['hotline'])];
                } elseif (!empty($facilityData['phones']) && is_array($facilityData['phones'])) {
                    $branchPhonesArray = array_map('trim', array_filter($facilityData['phones'], fn($p) => !empty($p)));
                } else {
                    $branchPhonesArray = null;
                }
            }
    ...
    {
        $jsonFilePath = base_path('database/seeders/AlexandriaData/alexandria.json');
