...
This folder contains seeders for importing medical facilities data for Cairo Governorate from JSON format.

## Structure

- `GovernorateSeeder.php` - Creates Cairo governorate
...
- `CairoDataSeeder.php` - Main seeder that runs all seeders in correct order
- `cairo.json` - JSON data file containing all facility information in nested associated array format
...
```bash
php artisan db:seed --class=Database\\Seeders\\CairoData\\CairoDataSeeder
```
...
```bash
# Create governorate
php artisan db:seed --class=Database\\Seeders\\CairoData\\GovernorateSeeder

# Create facility types
php artisan db:seed --class=Database\\Seeders\\CairoData\\FacilityTypeSeeder

# Create facilities
php artisan db:seed --class=Database\\Seeders\\CairoData\\FacilitySeeder

# Create branches
php artisan db:seed --class=Database\\Seeders\\CairoData\\FacilityBranchSeeder
```
...
- **Branches Without Phones**: Handles branches that only have location (no phone field) - phone will be taken from facility's phones/hotline
...
- **Facilities with Phones Array and Branches**: Facilities can have both a `phones` array (for the main facility) and a `branches` array (for branch locations)
...
- Facilities can have both `phones` array (for main facility phone) and `branches` array (for branch locations) - branches without phones will use facility's phones/hotline
