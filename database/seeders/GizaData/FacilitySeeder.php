...
class FacilitySeeder extends Seeder
{
    ...
    public function run(): void
    {
        try {
            ...
            if (!$this->giza) {
                $this->command->error('Giza governorate not found. Please run GovernorateSeeder first.');
                return;
            }
            ...
            $this->command->info("Starting facility import for Giza...");
            ...
            foreach ($jsonData['medical_directory'] as $category) {
                ...
                if (!empty($category['facilities']) && is_array($category['facilities'])) {
                    foreach ($category['facilities'] as $facilityData) {
                        ...
                        $this->createFacility($facilityData, $facilityType, $created, $updated);
                    }
                }
                ...
                if (!empty($category['chains']) && is_array($category['chains'])) {
                    foreach ($category['chains'] as $chainData) {
                        ...
                        // Treat chains as facilities with locations array
                        $this->createFacility($chainData, $facilityType, $created, $updated);
                    }
                }
            }
     ...
     */
    protected function createFacility(array $facilityData, $facilityType, &$created, &$updated): void
    {
        ...
        $hasLocations = !empty($facilityData['locations']) && is_array($facilityData['locations']);
        
        if (!$hasBranches && !$hasLocations) {
