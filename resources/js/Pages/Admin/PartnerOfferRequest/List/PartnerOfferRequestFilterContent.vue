      ...
      <div class="flex-1 min-w-0">
        ...
        <div class="relative">
          ...
          <input
            ...
            id="search"
            :placeholder="t.partner_offer_request?.search_placeholder || 'Search by phone number...'"
          />
        </div>
      </div>
    </div>
...
const props = defineProps({
  initialFilters: {
    type: Object,
    default: () => ({ search: '', partner_offer_id: '' }),
  },
});
...
const getInitialFilters = () => ({
  ...
  partner_offer_id: props.initialFilters?.partner_offer_id || '',
});
...
const handleReset = () => {
  filters.value = { search: '', partner_offer_id: '' };
  applyFilters();
};
...
const applyFilters = (filterValues = null) => {
  ...
  if (currentFilters.partner_offer_id) params.partner_offer_id = currentFilters.partner_offer_id;
  emit('filter-change', currentFilters);
  router.get(route('admin.partner-offer-request.list'), params, {
    preserveState: true,
    preserveScroll: true,
    replace: true,
  });
