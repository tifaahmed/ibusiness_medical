    ...
    <div class="px-6">
      ...
      <div v-if="showAddForm || editingIndex !== null" class="mb-6 p-4 bg-accent/50 rounded-lg border border-border">
        <h3 class="text-sm font-semibold mb-4 text-white">
          {{ editingIndex !== null ? (t.member?.edit_family_member || 'Edit Family Member') : (t.member?.add_new_family_member || 'Add New Family Member') }}
        </h3>
        <form @submit.prevent="handleSubmit" class="space-y-4">
          ...
          <div class="flex gap-3 justify-end">
            ...
            >
              {{ editingIndex !== null ? (t.member?.update_family_member || 'Update Family Member') : (t.member?.add_family_member || 'Add Family Member') }}
            </button>
          </div>
        </form>
      </div>
        ...
        >
          <div class="flex items-start justify-between gap-4">
            <div class="flex items-start gap-4 flex-1">
              <div v-if="member.photo_preview" class="flex-shrink-0">
                <img
              ...
              <div class="flex-1 min-w-0">
                ...
                <p class="text-sm text-white/80 mb-2">
                  <span class="font-medium">{{ getRelationshipLabel(member.relationship) }}</span>
...
const editMember = (index) => {
  ...
  const member = props.modelValue[index];
  form.value = {
    name: member.name || '',
    ...
    photo_preview: member.photo_preview || null,
    photo_file: member.photo_file || null
  };
  photoFile.value = member.photo_file || null;
  errors.value = {};
};
...
const handleSubmit = () => {
  ...
  // Basic validation
  if (!form.value.name) {
    errors.value.name = t.value.member?.name_required || 'Name is required';
    return;
  }
  if (!form.value.relationship) {
    errors.value.relationship = t.value.member?.relationship_required || 'Relationship is required';
    return;
  }
  ...
  const memberData = {
    ...
    photo_preview: form.value.photo_preview,
    photo_file: form.value.photo_file
  };

  const currentMembers = [...props.modelValue];

  if (editingIndex.value !== null) {
    // Update existing member
    currentMembers[editingIndex.value] = memberData;
  } else {
    // Add new member
    currentMembers.push(memberData);
  }

  emit('update:modelValue', currentMembers);
  cancelForm();
};

const deleteMember = (index) => {
  if (!confirm(t.value.member?.confirm_remove_family_member || 'Are you sure you want to remove this family member?')) {
    return;
  }
