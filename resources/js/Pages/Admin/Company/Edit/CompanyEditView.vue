<template>
  <CompanyLayout>
    <div class="space-y-4 p-2 sm:p-4 lg:p-6 w-full max-w-full">
      <Breadcrumb
        title="Edit Company"
        :breadcrumbs="[{ label: 'Companies', link: route('admin.company.list'), active: false }, { label: 'Edit Company', link: '#', active: true }]"
      />
            ...
            <div class="flex flex-col sm:flex-row p-4 gap-3">
              ...
              <div class="flex gap-3 justify-end">
                ...
                <div class="relative inline-flex">
                  ...
                  <ErrorTrackButton :errors="companyStore.validationErrors || {}" :debug-log="companyStore.debugLog" />
                </div>
              </div>
            </div>
          </div>
...
watch(() => props.company, (val) => {
  if (val?.id) companyStore.setCompany(val);
}, { immediate: true, deep: true });

const handleSubmit = () => companyStore.updateCompany();
</script>
