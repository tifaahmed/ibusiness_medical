<template>
  <button
    ...
    title="Generate the missing meta title / description / keywords across all facilities"
    @click="openDialog"
  >
    ...
    >
      <div class="w-full max-w-lg overflow-hidden rounded-xl border border-border bg-card text-card-foreground shadow-xl">
        <div class="flex items-start gap-3 border-b border-border p-4">
          <div class="min-w-0">
            <h2 class="text-base font-semibold">Fill facility SEO with AI</h2>
            <p class="text-xs text-muted-foreground">
              AI writes bilingual meta title / description / keywords for facilities that are missing them.
              Review each facility afterwards.
            </p>
          </div>
          <button
            type="button"
          ...
          <div v-if="phase === 'idle'" class="space-y-3">
            <label class="flex items-start gap-2 text-sm">
              ...
              <span>
                ...
                <span class="block text-[11px] text-muted-foreground">Only facilities where meta title or description is completely empty.</span>
              </span>
            </label>
            <label class="flex items-start gap-2 text-sm">
              <input type="checkbox" v-model="doOg" class="mt-0.5" />
              <span>
                Copy the cover image into an empty share image
                <span class="block text-[11px] text-muted-foreground">Facilities with a cover image but no Open Graph image.</span>
              </span>
            </label>
            <label class="flex items-start gap-2 text-sm">
              <input type="checkbox" v-model="overwrite" class="mt-0.5" />
              <span>
                Regenerate SEO for every facility
...
// same slice rather than burning through the rest of the list with errors. If
// the window has not cleared yet the retry 429s again and we simply wait again.
...
const start = async () => {
  ...
  try {
    const { data } = await axios.post(route("admin.facility.seo.bulk.begin"), {
    ...
    // Units of work = SEO jobs + image jobs (a facility needing both counts twice).
    totalUnits.value = seoSlugs.value.length + ogSlugs.value.length;

    if (totalUnits.value === 0) {
      phase.value = "done";
      useNotification().success("Nothing to do — every facility already has what you selected.");
      return;
    }
...
const runQueue = async () => {
  ...
  while (i < allSlugs.length) {
    ...
    let data;
    try {
      ({ data } = await axios.post(route("admin.facility.seo.bulk.step"), {
        slugs: batch,
