  ...
  <FacilityLayout>
    <div class="space-y-3 p-3 sm:p-4 lg:p-6 w-full max-w-full">
      ...
      <div v-if="step === 'upload'" class="bg-card border border-border rounded-xl p-6 space-y-4 max-w-2xl mx-auto">
        <h2 class="text-lg font-semibold">Upload facilities file</h2>
        <p class="text-sm text-muted-foreground">
          Upload a CSV or XLSX file. The expected format matches the export — the same column headers
          (<code>Name, Name (AR), Slug, Facility type, Governorate, City, Latitude, Longitude</code>).
          An optional <code>Branches</code> sheet can include rows keyed by <code>Facility name</code> or <code>Facility slug</code>.
        </p>
        <input
          type="file"
      ...
      <div v-if="step === 'preview'" class="space-y-4">
        <div class="bg-card border border-border rounded-xl p-4 flex flex-wrap items-center gap-4">
          <div class="flex items-center gap-2">
            ...
            <span v-if="totalBranches" class="px-2.5 py-1 rounded text-xs bg-violet-600 text-white font-semibold">{{ totalBranches }} branches</span>
          </div>
        ...
        <div v-if="mode === 'clear'" class="bg-red-600 text-white border border-red-700 rounded-md p-3 text-sm">
          <strong>Warning:</strong> "Clear table" deletes every existing facility and its branches. Imported rows are then inserted as new facilities.
        </div>

        <div class="bg-card text-card-foreground border border-border rounded-xl overflow-x-auto">
          <table class="text-xs text-foreground" style="min-width: 1500px; width: 100%;">
            <thead class="bg-muted/50">
              <tr class="border-b border-border">
                ...
                <th class="px-2 py-2 text-left font-semibold w-56">Name</th>
                <th class="px-2 py-2 text-left font-semibold w-44">Name (AR)</th>
                ...
                <th class="px-2 py-2 text-left font-semibold w-28">Latitude</th>
                <th class="px-2 py-2 text-left font-semibold w-28">Longitude</th>
              ...
              <template v-for="(row, i) in rows" :key="i">
                <tr class="border-b border-border" :class="rowBgClass(row.status)">
                  <td class="px-2 py-1.5 text-center font-mono sticky left-0 z-[5] bg-card" :class="rowBgClass(row.status)">
                    {{ i + 1 }}
                    <button
                      v-if="row.branches && row.branches.length"
                      type="button"
                      @click="row.showBranches = !row.showBranches"
                      class="block mx-auto mt-0.5 text-[10px] px-1.5 py-0.5 rounded bg-violet-600 text-white font-semibold"
                      :title="row.showBranches ? 'Hide branches' : 'Show branches'"
                    >
                      {{ row.branches.length }} br
                    </button>
                  </td>
                ...
                <!-- Branches sub-table -->
                <tr v-if="row.showBranches && row.branches && row.branches.length" :class="rowBgClass(row.status)">
                  <td colspan="10" class="p-3">
                    <div class="bg-card border border-violet-500 rounded-md overflow-hidden">
                      <div class="bg-violet-600 text-white px-3 py-2 text-xs font-semibold">
                        Branches for {{ row.parsed.name }} ({{ row.branches.length }})
                      </div>
                      <table class="w-full text-[11px]">
                        ...
                        <tbody>
                          <tr v-for="(br, bi) in row.branches" :key="bi" class="border-t border-border">
                            <td class="px-3 py-1"><input v-model="br.name" :class="inputCls()" /></td>
                            <td class="px-3 py-1"><input v-model="br.name_ar" :class="inputCls()" /></td>
                            ...
                            <td class="px-3 py-1 text-center">
                              <button type="button" @click="row.branches.splice(bi, 1)" class="text-red-600 hover:underline text-[10px]">remove</button>
                            </td>
                          </tr>
                        </tbody>
                      </table>
      ...
      <div v-if="step === 'done'" class="bg-card border border-border rounded-xl p-6 max-w-2xl mx-auto text-center space-y-4">
        ...
        <div class="text-sm">
          ...
          <span v-if="result?.branches" class="px-2 py-1 rounded bg-violet-500/15 text-violet-700 dark:text-violet-300 font-semibold mr-2">{{ result.branches }} branches</span>
        ...
        <div class="flex justify-center gap-2">
          <Link :href="route('admin.facility.list')" class="inline-flex items-center justify-center rounded-md text-sm font-medium bg-primary text-primary-foreground h-9 px-4 btn-golden">
            Go to facilities
          </Link>
...
const totalBranches = computed(() => rows.value.reduce((sum, r) => sum + (r.branches?.length || 0), 0));
...
const uploadFile = async () => {
  ...
  try {
    ...
    const { data } = await axios.post(route('admin.facility.import.preview'), fd);
    rows.value = (data.rows || []).map(r => ({
      ...r,
      showDiff: false,
      showBranches: (r.branches || []).length > 0,
      branches: (r.branches || []).map(b => ({ ...b, governorate_id: null, city_id: null })),
    }));
    facilityTypeOptions.value = data.facilityTypeOptions || [];
  ...
  try {
    const payload = {
      ...
      rows: rows.value.map(r => ({
        ...
        facility_type_id: r.parsed.facility_type_id,
        governorate_id: r.parsed.governorate_id,
        ...
        branches: (r.branches || []).map(b => ({
          ...
          latitude: b.latitude !== '' && b.latitude !== null ? Number(b.latitude) : null,
          longitude: b.longitude !== '' && b.longitude !== null ? Number(b.longitude) : null,
        })),
      })),
    };
    const { data } = await axios.post(route('admin.facility.import.commit'), payload);
    result.value = data;
    step.value = 'done';
  } catch (e) {
...
const citiesForBranch = (br) => {
  const gov = br.governorate_id;
  if (!gov) return cityOptions.value;
  return cityOptions.value.filter(c => String(c.governorate_id) === String(gov));
};
