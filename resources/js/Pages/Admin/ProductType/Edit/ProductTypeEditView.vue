<template>
  <ProductTypeLayout>
    <div class="space-y-2 sm:space-y-3 md:space-y-4 p-2 sm:p-3 md:p-4 lg:p-6 w-full max-w-full">
      <Breadcrumb
        :title="t.product_type?.edit || 'Edit Product Type'"
        :breadcrumbs="[
          { label: t.breadcrumbs?.product_types || 'Product Types', link: route('admin.product-type.list'), active: false },
          { label: t.product_type?.edit || 'Edit Product Type', link: '#', active: true },
        ]"
      />
            ...
            <div class="flex flex-col sm:flex-row p-2 sm:p-3 md:p-4 gap-2 sm:gap-3">
              ...
              <div class="flex gap-2 sm:gap-3 justify-end">
                ...
                <div class="relative inline-flex">
                  <button
                    type="submit"
                    :disabled="productTypeStore.form.processing"
                  ...
                  >
                    ...
                    {{ t.product_type?.update || 'Update Product Type' }}
                  </button>
                  <ErrorTrackButton :errors="productTypeStore.validationErrors || {}" :debug-log="productTypeStore.debugLog" />
                </div>
              </div>
            </div>
          </div>
...
import { ProductTypeForm } from "../_components/Form";
...
// Set product type data when props are received
// Use watch with immediate to ensure it runs on mount and when props change
watch(() => props.productType, (newProductType) => {
  if (newProductType && newProductType.id) {
    productTypeStore.setProductType(newProductType);
  }
}, { immediate: true, deep: true });

const handleSubmit = () => {
  productTypeStore.updateProductType();
};
</script>
