<template>
  <div data-slot="card" class="bg-card text-card-foreground flex flex-col gap-4 rounded-xl border border-border py-4 shadow-sm">
    <div v-if="productTypes?.data?.length > 0" data-slot="card-content" class="p-0">
              ...
              >
                    ...
                    <div class="flex-1 min-w-0 space-y-2">
                      ...
                      <div class="flex items-center gap-2 mt-1">
                        <div class="flex items-center gap-1 text-xs text-muted-foreground">
                          <span class="font-mono">{{ productType.slug }}</span>
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                <td data-slot="table-cell" class="p-2 align-middle whitespace-nowrap [&:has([role=checkbox])]:pr-0 [&>[role=checkbox]]:translate-y-[2px] text-center">
                  <div class="flex items-center justify-center gap-2">
                    ...
                    <button
                      :disabled="!productType.slug"
                      @click="productType.slug && $emit('delete', productType.slug)"
          ...
          <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between w-full">
            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-6 w-full">
              <div class="text-xs sm:text-sm text-muted-foreground order-2 sm:order-1">
                {{ (t.common?.showing_results || 'Showing :from to :to of :total results').replace(':from', productTypes.meta?.from || 0).replace(':to', productTypes.meta?.to || 0).replace(':total', productTypes.meta?.total || 0) }}
              </div>
            </div>
            <div>
              <div class="flex items-center gap-2 order-1 sm:order-2">
                ...
                <PerPageSelect
                  :model-value="productTypes.meta?.per_page || 15"
                  @update:model-value="handlePerPageChange"
                />
              </div>
            </div>
            <Pagination
              v-if="productTypes?.meta?.links?.length > 0"
              :links="productTypes?.meta?.links"
              class="order-3"
            />
          </div>
        </div>
    ...
    <div v-else data-slot="card-content" class="p-12">
      <div class="text-center max-w-md mx-auto space-y-6">
        <div class="inline-flex items-center justify-center w-20 h-20 sm:w-24 sm:h-24 rounded-full bg-golden-yellow/10 border border-golden-yellow/20 mb-6 shadow-lg shadow-golden-yellow/10">
          <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-package w-10 h-10 sm:w-12 sm:h-12 text-golden-yellow subtle-float">
        ...
        >
          ...
          {{ t.product_type?.add || 'Add Product Type' }}
        </Link>
      </div>
    </div>
  </div>
...
const getEditRoute = (slug) => {
  ...
  try {
    return route('admin.product-type.edit', slug);
  } catch (error) {
