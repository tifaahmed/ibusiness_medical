    ...
    <div class="container mx-auto px-4 py-6 md:px-6 lg:px-8 relative z-10">
      <div class="space-y-4">
        <form class="space-y-3" @submit.prevent="handleSubmit">
          <ServiceTypeForm :service-type="serviceType" :icon-options="iconOptions" :color-options="colorOptions" />
            ...
            <div class="flex flex-col sm:flex-row p-4">
              ...
              <div class="flex gap-3 justify-end">
                ...
                <div class="relative inline-flex order-1 sm:order-2">
                  <button
                    type="submit"
                    :disabled="serviceTypeStore.form.processing"
                    data-slot="button"
                  ...
                  >
                    ...
                    {{ t.serviceType?.update || 'Update Category' }}
                  </button>
                  <ErrorTrackButton :errors="serviceTypeStore.validationErrors || {}" :debug-log="serviceTypeStore.debugLog" />
                </div>
              </div>
            </div>
          </div>
...
import ServiceTypeForm from "../Form/ServiceTypeForm.vue";
...
onMounted(() => {
  serviceTypeStore.setServiceType(props.serviceType);
});

watch(() => props.serviceType, (newServiceType) => {
  if (newServiceType && newServiceType.id) serviceTypeStore.setServiceType(newServiceType);
}, { deep: true });

const handleSubmit = () => {
  serviceTypeStore.updateServiceType();
};
</script>
