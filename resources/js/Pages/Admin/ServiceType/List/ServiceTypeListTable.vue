<template>
  <div data-slot="card" class="bg-card text-card-foreground flex flex-col h-full lg:h-auto rounded-xl border border-border shadow-sm">
    <div v-if="serviceTypes?.data?.length > 0" class="flex flex-col h-full lg:h-auto min-h-0 lg:min-h-fit">
            ...
            <thead data-slot="table-header" class="[&_tr]:border-b [&_tr]:border-border">
              <tr data-slot="table-row" class="hover:bg-muted/50 border-b border-border transition-colors">
                <th data-slot="table-head" class="text-foreground h-9 sm:h-10 px-2 sm:px-3 text-left align-middle font-medium whitespace-nowrap min-w-[200px] sm:min-w-[300px]">
                  {{ t.serviceType?.details || 'Category Details' }}
                </th>
                <th data-slot="table-head" class="text-foreground h-9 sm:h-10 px-2 sm:px-3 align-middle font-medium whitespace-nowrap w-20 sm:w-28 text-center hidden sm:table-cell">
                  {{ t.serviceType?.status || 'Status' }}
                </th>
                <th data-slot="table-head" class="text-foreground h-9 sm:h-10 px-2 sm:px-3 align-middle font-medium whitespace-nowrap w-20 sm:w-28 text-center hidden md:table-cell">
                  {{ t.serviceType?.services_count || 'Services' }}
                </th>
              ...
              >
                <td data-slot="table-cell" class="p-2 sm:p-3 align-middle">
                  <div class="flex items-start gap-2 sm:gap-4">
                    <div v-if="serviceType.image" class="flex-shrink-0 hidden sm:block">
                      <img :src="serviceType.image" :alt="nameOf(serviceType)" class="w-12 h-12 object-cover rounded-lg border border-border" />
                    </div>
                    <div v-else-if="serviceType.icon" class="flex-shrink-0 hidden sm:flex items-center justify-center w-10 h-10 rounded-lg border border-border" :style="{ backgroundColor: serviceType.color || '#f0f0f0' }">
                      <span class="text-xl">{{ serviceType.icon }}</span>
                    </div>
                    <div class="flex-1 min-w-0 space-y-1 sm:space-y-1.5">
                      <Link
                        :href="getEditRoute(serviceType.id)"
                        ...
                        :title="nameOf(serviceType)"
                      >
                        {{ serviceType.name?.en || '-' }}
                      </Link>
                      <div v-if="serviceType.name?.ar" dir="rtl" class="text-xs text-muted-foreground max-w-[200px] break-words">
                        {{ serviceType.name.ar }}
                      </div>
                      <div v-if="serviceType.description" class="max-w-[200px] truncate text-xs text-muted-foreground">
                ...
                <td data-slot="table-cell" class="p-2 align-middle whitespace-nowrap text-center hidden md:table-cell">
                  <span class="text-muted-foreground text-sm">{{ serviceType.services_count || 0 }}</span>
                </td>
                <td data-slot="table-cell" class="p-2 align-middle whitespace-nowrap text-center">
                  <div class="flex items-center justify-center gap-2">
                    <Link
                      ...
                      :class="{ 'opacity-50 cursor-not-allowed pointer-events-none': !serviceType.id }"
                    ...
                    <button
                      ...
                      @click="serviceType.id && $emit('delete', serviceType.id)"
        ...
        <div class="border-t border-border/50 px-3 sm:px-4 md:px-6 py-2.5 sm:py-3 md:py-4 w-full">
          <div class="flex flex-row items-center justify-between gap-2 sm:gap-3 lg:gap-4 w-full flex-wrap">
            <div class="text-xs sm:text-sm text-muted-foreground order-1 flex-shrink-0 min-w-0">
              <span class="hidden sm:inline">{{ (t.common?.showing_results || 'Showing :from to :to of :total results').replace(':from', serviceTypes.meta?.from || 0).replace(':to', serviceTypes.meta?.to || 0).replace(':total', serviceTypes.meta?.total || 0) }}</span>
              <span class="sm:hidden">{{ serviceTypes.meta?.from || 0 }}-{{ serviceTypes.meta?.to || 0 }}/{{ serviceTypes.meta?.total || 0 }}</span>
            </div>
            <div class="flex items-center gap-2 order-2 flex-shrink-0">
              ...
              <PerPageSelect
                :model-value="serviceTypes.meta?.per_page || 15"
                @update:model-value="handlePerPageChange"
              />
            </div>
            <div class="order-3 flex-shrink-0 min-w-0">
              <Pagination
                v-if="serviceTypes?.meta?.links?.length > 0"
                :links="serviceTypes?.meta?.links"
              />
            </div>
          </div>
        </div>
    ...
    <div v-else data-slot="card-content" class="p-12">
      <div class="text-center max-w-md mx-auto space-y-6">
        ...
        >
          ...
          {{ t.serviceType?.add || 'Add Category' }}
        </Link>
      </div>
    </div>
  </div>
...
const nameOf = (serviceType) => serviceType.name?.en || serviceType.name?.ar || '-';

const getEditRoute = (id) => {
  if (!id) return route('admin.service-type.list');
  try {
    return route('admin.service-type.edit', id);
  } catch (error) {
    console.error('Error generating edit route:', error);
    return route('admin.service-type.list');
  }
};
