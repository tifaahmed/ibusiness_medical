          ...
          <div class="flex flex-row items-center justify-between py-2 px-3 sm:px-4 md:px-6 gap-2 sm:gap-4">
            <div class="leading-none font-semibold min-w-0 flex-1">
              <div class="title-golden flex items-center gap-2 min-w-0">
                ...
                <span class="text-sm sm:text-base truncate min-w-0">
                  Activity Logs &mdash; {{ branchName }}
                </span>
              </div>
            </div>
            <div class="flex items-center gap-2 flex-shrink-0">
              ...
              >
                ...
                <span class="hidden sm:inline">Edit Branch</span>
              ...
              >
                ...
                <span class="hidden sm:inline">Back to Branches</span>
        ...
        <div data-slot="card" class="bg-card text-card-foreground rounded-xl border border-border shadow-sm">
                    ...
                    <tr v-if="expanded[log.id]" class="bg-muted/30 border-b border-border">
                      <td colspan="5" class="p-3 sm:p-4 space-y-4">
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                          <div>
                            <div class="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-2">Before</div>
                          ...
                          <div>
                            <div class="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-2">After</div>
          ...
          <div v-else class="p-12 text-center">
            ...
            <p class="text-muted-foreground text-sm">
              No audit log entries exist for this branch. New entries will appear here as changes are made.
            </p>
          </div>
        </div>
      </div>
...
const props = defineProps({
  facilityBranch: { type: Object, required: true },
  logs: { type: Object, required: true },
  filters: { type: Object, default: () => ({ admin_id: null, action: null }) },
...
const branchName = computed(() => {
  const n = props.facilityBranch?.name;
  if (!n) return props.facilityBranch?.slug || '';
  if (typeof n === 'string') return n;
  if (typeof n === 'object') {
    return n[locale.value] || n.ar || n.en || Object.values(n)[0] || '';
  }
  return String(n);
});
...
const applyFilters = () => {
  ...
  router.get(route('admin.facility-branch.logs', props.facilityBranch.slug), params, {
    preserveState: true,
    preserveScroll: true,
    replace: true,
  });
