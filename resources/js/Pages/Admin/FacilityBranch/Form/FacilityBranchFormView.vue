                ...
                <div class="flex gap-3 justify-end">
                  ...
                  <div class="relative inline-flex order-1 sm:order-2">
                    ...
                    >
                      ...
                      {{ isEditMode ? (t.facility_branch?.update || 'Update Branch') : (t.facility_branch?.create || 'Create Branch') }}
                    </button>
                    <ErrorTrackButton :errors="facilityBranchStore.validationErrors || {}" :debug-log="facilityBranchStore.debugLog" />
                  </div>
                </div>
              </div>
            </div>
...
import FacilityBranchForm from "./FacilityBranchForm.vue";
...
const isEditMode = computed(() => !!props.facilityBranch?.id);

// Initialize form based on mode
onMounted(() => {
  if (isEditMode.value) {
    facilityBranchStore.setFacilityBranch(props.facilityBranch);
  } else {
    facilityBranchStore.initializeForm();
  }
});

// Watch for changes in edit mode (prop updates)
watch(() => props.facilityBranch, (newFacilityBranch) => {
  if (newFacilityBranch && newFacilityBranch.id) {
    facilityBranchStore.setFacilityBranch(newFacilityBranch);
  }
}, { deep: true });
