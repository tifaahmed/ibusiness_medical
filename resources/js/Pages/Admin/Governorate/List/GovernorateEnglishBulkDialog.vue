<template>
  <button
    ...
    title="Translate / fix empty or Arabic English names across all governorates"
    @click="openDialog"
  >
...
const start = async () => {
  ...
  try {
    const { data } = await axios.post(route("admin.governorate.english.bulk.begin"));
    ...
    if (total.value === 0) {
      phase.value = "done";
      useNotification().success("Every governorate's English name is already clean.");
      return;
    }
    ...
    while (i < slugs.value.length) {
      ...
      let res;
      try {
        res = await axios.post(route("admin.governorate.english.bulk.step"), { slugs: batch });
      } catch (error) {
