    ...
    <div class="container mx-auto px-4 py-6 md:px-6 lg:px-8 relative z-10">
      <div class="space-y-4">
        <form class="space-y-3" @submit.prevent="handleSubmit">
          <NewsTickerForm :news-ticker="newsTicker" />
            ...
            <div class="flex flex-col sm:flex-row p-4">
              ...
              <div class="flex gap-3 justify-end">
                ...
                <div class="relative inline-flex order-1 sm:order-2">
                  <button
                    type="submit"
                    :disabled="newsTickerStore.form.processing"
                    data-slot="button"
                  ...
                  >
                    ...
                    {{ isEditMode ? (t.news_ticker?.update || 'Update News') : (t.news_ticker?.create || 'Create News') }}
                  </button>
                  <ErrorTrackButton :errors="newsTickerStore.validationErrors || {}" :debug-log="newsTickerStore.debugLog" />
                </div>
              </div>
            </div>
          </div>
...
import NewsTickerForm from "./NewsTickerForm.vue";
...
const isEditMode = computed(() => !!props.newsTicker?.id);

onMounted(() => {
  if (isEditMode.value) {
    newsTickerStore.setNewsTicker(props.newsTicker);
  } else {
    newsTickerStore.initializeForm();
  }
});

watch(() => props.newsTicker, (newVal) => {
  if (newVal && newVal.id) newsTickerStore.setNewsTicker(newVal);
}, { deep: true });

const handleSubmit = () => {
  if (isEditMode.value) {
    newsTickerStore.updateNewsTicker();
  } else {
    newsTickerStore.submitForm();
  }
};
</script>
