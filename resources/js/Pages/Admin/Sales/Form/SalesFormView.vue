            ...
            <div class="flex flex-col sm:flex-row p-4">
              ...
              <div class="flex gap-3 justify-end">
                ...
                <div class="relative inline-flex order-1 sm:order-2">
                  <button
                    type="submit"
                    :disabled="salesStore.form.processing"
                    data-slot="button"
                  ...
                  >
                    ...
                    {{ isEditMode ? (t.sales?.update || 'Update Sales') : (t.sales?.create || 'Create Sales') }}
                  </button>
                  <ErrorTrackButton :errors="salesStore.validationErrors || {}" :debug-log="salesStore.debugLog" />
                </div>
              </div>
            </div>
          </div>
...
onMounted(() => {
  if (isEditMode.value) {
    salesStore.setSale(props.sale);
  } else {
    salesStore.initializeForm();
  }
});

watch(() => props.sale, (newSale) => {
  if (newSale && newSale.id) salesStore.setSale(newSale);
}, { deep: true });
