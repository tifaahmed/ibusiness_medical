<template>
  <button
    ...
    title="Translate / fix empty or Arabic English title, short description and full description across all offers"
    @click="openDialog"
  >
      ...
      <div class="w-full max-w-lg overflow-hidden rounded-xl border border-border bg-card text-card-foreground shadow-xl">
        <div class="flex items-start gap-3 border-b border-border p-4">
          <div class="min-w-0">
            ...
            <p class="text-xs text-muted-foreground">
              AI fills or repairs the English title, short description and full description that are empty,
        ...
        <div class="max-h-[60vh] overflow-y-auto p-4 space-y-3">
          <p v-if="phase === 'idle'" class="text-sm text-muted-foreground">
            This scans every offer you can manage and fixes the ones with dirty English data.
          </p>
...
const start = async () => {
  ...
  try {
    const { data } = await axios.post(route("admin.offer.english.bulk.begin"));
    ...
    if (total.value === 0) {
      phase.value = "done";
      useNotification().success("Every offer's English data is already clean.");
      return;
    }
    ...
    while (i < slugs.value.length) {
      ...
      let res;
      try {
        res = await axios.post(route("admin.offer.english.bulk.step"), { slugs: batch });
      } catch (error) {
