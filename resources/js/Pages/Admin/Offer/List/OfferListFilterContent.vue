    ...
    <div class="flex flex-col sm:flex-row sm:items-end gap-2 sm:gap-3 w-full">
      ...
      <div class="flex-1 min-w-0">
        ...
        <div class="relative">
          ...
          <input
            ...
            id="search"
            :placeholder="t.offer?.search_placeholder || 'Search by title, phone, price...'"
          />
        </div>
      </div>
      ...
      <div class="w-full sm:w-48">
        <label
          ...
          for="offerable_type"
        >
          {{ t.offer?.type || 'Offer Type' }}
        </label>
        <Select
          :key="`offerable-type-${locale}`"
          id="offerable_type"
          v-model="filters.offerable_type"
...
const offerTypeOptions = computed(() => [
  { value: 'App\\Models\\Facility', label: t.value.facility?.label || 'Facility' },
  { value: 'App\\Models\\FacilityBranch', label: t.value.facility_branch?.label || 'Facility Branch' }
]);
...
const getInitialFilters = () => {
  if (props.initialFilters) {
    return {
      ...
      offerable_type: props.initialFilters.offerable_type || ''
    };
  }
  ...
  if (typeof window !== 'undefined') {
    ...
    return {
      ...
      offerable_type: urlParams.get('offerable_type') || ''
    };
  }
...
const hasActiveFilters = computed(() => {
  return !!(filters.value.search || filters.value.offerable_type);
});
...
const applyFilters = (filterValues = null) => {
  ...
  if (currentFilters.offerable_type && currentFilters.offerable_type !== '') {
    params.offerable_type = currentFilters.offerable_type;
  }

  emit('filter-change', currentFilters);

  router.get(route('admin.offer.list'), params, {
    preserveState: true,
    preserveScroll: true,
    replace: true
  });
