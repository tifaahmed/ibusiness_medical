    ...
    <div class="container mx-auto px-4 py-6 md:px-6 lg:px-8 relative z-10">
      <div class="space-y-4">
        <form class="space-y-3" @submit.prevent="handleSubmit">
          <PartnerOfferForm :partner-offer="partnerOffer" :partners="partners" :operators="operators" />
            ...
            <div class="flex flex-col sm:flex-row p-4">
              ...
              <div class="flex gap-3 justify-end">
                ...
                <div class="relative inline-flex order-1 sm:order-2">
                  <button
                    type="submit"
                    :disabled="partnerOfferStore.form.processing"
                    data-slot="button"
                  ...
                  >
                    ...
                    {{ isEditMode ? (t.partner_offer?.update || 'Update Offer') : (t.partner_offer?.create || 'Create Offer') }}
                  </button>
                  <ErrorTrackButton :errors="partnerOfferStore.validationErrors || {}" :debug-log="partnerOfferStore.debugLog" />
                </div>
              </div>
            </div>
          </div>
...
import PartnerOfferForm from "./PartnerOfferForm.vue";
...
onMounted(() => {
  if (isEditMode.value) {
    partnerOfferStore.setPartnerOffer(props.partnerOffer);
  } else {
    partnerOfferStore.initializeForm();
  }
});

watch(() => props.partnerOffer, (newOffer) => {
  if (newOffer && newOffer.id) partnerOfferStore.setPartnerOffer(newOffer);
}, { deep: true });

const handleSubmit = () => {
  if (isEditMode.value) {
    partnerOfferStore.updatePartnerOffer();
  } else {
    partnerOfferStore.submitForm();
  }
};
</script>
