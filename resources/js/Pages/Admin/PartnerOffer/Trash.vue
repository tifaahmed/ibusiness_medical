  ...
  <PartnerLayout>
    <div class="flex flex-col h-full lg:h-auto w-full max-w-full overflow-x-hidden">
          ...
          <div data-slot="card-header" class="flex flex-row items-center justify-between py-2 px-3 sm:px-4 md:px-6 w-full overflow-hidden gap-2 sm:gap-4">
            <div data-slot="card-title" class="leading-none font-semibold min-w-0 flex-1">
              <div class="title-golden min-w-0 flex items-center gap-2">
                ...
                <span class="text-sm sm:text-base truncate block min-w-0">{{ t.partner_offer?.trash_title || 'Trash - Deleted Offers' }}</span>
              </div>
            </div>
            <Link
      ...
      <div class="flex-1 min-h-0 lg:min-h-fit w-full max-w-full px-2 sm:px-3 md:px-4 lg:px-6 pb-2 sm:pb-3 md:pb-4 lg:pb-6 overflow-hidden lg:overflow-visible">
        <TrashListTable :offers="offers" @restore="handleRestore" @force-delete="handleForceDelete" />
      </div>
    </div>
  </PartnerLayout>
</template>
...
import PartnerOfferListFilterContent from "./List/PartnerOfferListFilterContent.vue";
...
const handleRestore = (id) => {
  if (confirm('Are you sure you want to restore this partner offer?')) {
    router.post(route('admin.partner-offer.restore', id), {}, {
      preserveScroll: true,
    });
  }
};
...
const handleForceDelete = (id) => {
  if (confirm('Are you sure you want to permanently delete this partner offer? This action cannot be undone!')) {
    router.delete(route('admin.partner-offer.force-delete', id), {
      preserveScroll: true,
    });
  }
};
