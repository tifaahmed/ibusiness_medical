            ...
            <div class="flex flex-col sm:flex-row p-4">
              ...
              <div class="flex gap-3 justify-end">
                ...
                <div class="relative inline-flex order-1 sm:order-2">
                  <button
                    type="submit"
                    :disabled="partnerStore.form.processing"
                    data-slot="button"
                  ...
                  >
                    ...
                    {{ isEditMode ? (t.partner?.update || 'Update Partner') : (t.partner?.create || 'Create Partner') }}
                  </button>
                  <ErrorTrackButton :errors="partnerStore.validationErrors || {}" :debug-log="partnerStore.debugLog" />
                </div>
              </div>
            </div>
          </div>
...
onMounted(() => {
  if (isEditMode.value) {
    partnerStore.setPartner(props.partner);
  } else {
    partnerStore.initializeForm();
  }
});

watch(() => props.partner, (newPartner) => {
  if (newPartner && newPartner.id) partnerStore.setPartner(newPartner);
}, { deep: true });

const handleSubmit = () => {
  if (isEditMode.value) {
    partnerStore.updatePartner();
  } else {
    partnerStore.submitForm();
  }
};
</script>
